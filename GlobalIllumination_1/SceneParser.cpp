...
///Now, num_lights refers to number of lights which are not material, non-point lights. 
///For example, a ceiling light covered by a diffuse cover
...
SceneParser::SceneParser(const char* filename) {
    ...
    fclose(file); 
    file = NULL;
	num_lightSources = lightSources.size();
...
void SceneParser::parseFile() {
	...
	while (getToken(token)) {
		...
		//else if (!strcmp(token, "LightSources")) {parseLightSources();}
		else {
...
Material* SceneParser::parseMaterial() {
	...
	float bCoefficient = -1;
	float cCoefficient = 0;
	vector<float> diffuseRatio = vector<float>(32, 1.0f);
	vector<float> absorbRatio = vector<float>(32, 0);
	Vector3f lightColor{};
    getToken(token); assert (!strcmp(token, "{"));
	Noise *noise =0;
    while (1) {
		...
		else if (strcmp(token, "bCoefficient") == 0) {
			bCoefficient = readFloat();
		}
		else if (strcmp(token, "cCoefficient") == 0) {
			cCoefficient = readFloat();
		}
		///unimplemented
		...
		else if (strcmp(token, "diffuseRatio") == 0) {
			for (int w = 0; w < 32; w++)
			{
				diffuseRatio[w] = readFloat();
			}
		}
		else if (strcmp(token, "absorbRatio") == 0) {
			for (int w = 0; w < 32; w++)
			{
				absorbRatio[w] = readFloat();
			}
		}
		else {
            assert (!strcmp(token, "}"));
    ...
    Material *answer = new Material(diffuseRatio, absorbRatio, diffuseColor, specularColor, shininess,refractionIndex, lightColor, bCoefficient, cCoefficient);
	if(filename[0] !=0){
...
Object3D* SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
    ...
    } else if (!strcmp(token, "LightSource")) {
		answer = (Object3D*)parseLightSource();
	}
	else {
...
Object3D* SceneParser::parseLightSource() {

	char token[MAX_PARSER_TOKEN_LENGTH];
	getToken(token); assert(!strcmp(token, "{"));
	Object3D* answer = NULL;
	LightSource* lS = NULL;
	getToken(token); assert(!strcmp(token, "PhotonRatio"));
	float photonRatio = readFloat();
	bool v = true;
	getToken(token);
	if (!strcmp(token, "Invisible")) {
		v = false; getToken(token);
	}
	if (!strcmp(token, "Visible")) {
		v = true; getToken(token);
	}
	if (!strcmp(token, "RectangularLightSource")) {
		lS = parseRectangularLightSource(answer, photonRatio, v);
	}
	else {
		printf("Unknown token in parseLightSource: '%s'\n", token);
		exit(0);
	}
	getToken(token); assert(!strcmp(token, "}"));
...
LightSource* SceneParser::parseRectangularLightSource(Object3D* & o, float pR=1.0f, bool v = true) {
	char token[MAX_PARSER_TOKEN_LENGTH];
	Vector3f normal, side1, side2, vertex0;
	vector<float> spectrum = vector<float>(32, 0);
	float angle = 90.f;
	getToken(token); assert(!strcmp(token, "{"));
	while (true)
	{
		...
		else if (strcmp(token, "side1") == 0) {
			side1 = readVector3f();
		}
		else if (strcmp(token, "side2") == 0) {
			side2 = readVector3f();
		}
		else if (strcmp(token, "vertex0") == 0) {
			vertex0 = readVector3f();
		}
		else if (strcmp(token, "angle") == 0) { // degrees
			angle = readFloat();
		}
		else if (strcmp(token, "spectrum") == 0) {
			for (int w = 0; w < 32; w++)
			{
				spectrum[w] = readFloat();
			}
		}
		else if (!strcmp(token, "}")) {
			o = new Rectangle(vertex0, side1, side2, current_material, normal);
			return new RectangularLightSource(vertex0, side1, side2, normal, pR, spectrum, angle * M_PI/180, v); // radians
		}
		else {
			printf("Unknown token in parseRectangularLightSource: '%s'\n", token);
			exit(0);
		}
	}
}
