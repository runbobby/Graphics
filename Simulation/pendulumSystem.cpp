...
///Particle i+1 with a Hooke spring. 
///Drag force is - drag times the velocity. The other parameters should be self-explanatory
...
void PendulumSystem::setParameters(float s, float r, float g, float d, float m)
{
	if (s > 0) { springk = s; }
	if (r > 0) { rest_length = r; }
	if (g > 0) { gravity = g; }
	if (d > 0) { drag = d; }
	if (m > 0) { mass = m; }
}